export const initMagicCapture = (context, deps) => {
    ...
    const updateSelectionUi = () => {
        if (!(captureCropSelection instanceof HTMLElement))
            return;
        if (!(captureCropGuide instanceof HTMLElement))
            return;
        ...
        const geometry = resolveImageGeometry();
        if (!geometry || !(captureCropSize instanceof HTMLElement))
            return;
    ...
    const cropToDataUrl = () => {
        ...
        if (!ctx)
            return null;
        ctx.drawImage(captureCropImage, crop.x, crop.y, crop.width, crop.height, 0, 0, width, height);
    ...
    if (captureCropCanvas instanceof HTMLElement) {
        captureCropCanvas.addEventListener("pointerdown", (event) => {
            ...
            if ((target === captureCropSelection || (captureCropSelection === null || captureCropSelection === void 0 ? void 0 : captureCropSelection.contains(target))) &&
        ...
        captureCropCanvas.addEventListener("pointermove", (event) => {
            ...
            if (interactionMode === "idle") {
                ...
                else if (target === captureCropSelection ||
                    ((captureCropSelection === null || captureCropSelection === void 0 ? void 0 : captureCropSelection.contains(target)) && !target.classList.contains("capture-crop-handle"))) {
