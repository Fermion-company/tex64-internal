...
import type { MathWysiwygDeps } from "./types.js";

export type MathWysiwygPanelState = {
  deps: MathWysiwygDeps;
  panel: HTMLElement;
  panelHost: HTMLElement | null;
  active: boolean;
  explicitSession: boolean;
  explicitSessionPrefixLatex: string | null;
  selectedIndex: number;
  currentCandidates: Candidate[];
};

export type MathWysiwygPanelOps = {
  resolvePanelHost: () => HTMLElement | null;
  ensurePanel: () => void;
  setPanelVisible: (visible: boolean) => void;
  renderPanel: () => void;
  positionPanelNearCaret: () => void;
  scrollActiveIntoView: () => void;
  setApplyCandidateHandler: (handler: (index: number) => void) => void;
};

export const createMathWysiwygPanelOps = (state: MathWysiwygPanelState): MathWysiwygPanelOps => {
  ...
  const resolvePanelHost = () => {
    ...
    const host =
      (state.deps.container.closest(".panel-body.blocks-panel") as HTMLElement | null) ??
      (state.deps.container.closest(".panel-body") as HTMLElement | null) ??
      state.deps.container;
  ...
  const setPanelVisible = (visible: boolean) => {
    ...
    state.deps.container?.classList.toggle("has-wysiwyg-suggestions", visible);
    if (!visible) {
  ...
  const scrollActiveIntoView = () => {
    ...
    const activeItem = state.panel.querySelector(".math-wysiwyg-item.is-active") as HTMLElement | null;
    if (!activeItem) {
      return;
    }
    try {
  ...
  const setApplyCandidateHandler = (handler: (index: number) => void) => {
