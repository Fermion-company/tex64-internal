...
import type { LspBridge } from "./lsp-client.js";
...
export type SetupLspDeps = {
  getWorkspaceRoot: () => string | null;
};

export type LspHandle = {
  client: LspClient;
  docSync: LspDocumentSync;
  dispose: () => void;
};
...
export const setupLsp = async (monaco: Monaco, deps: SetupLspDeps): Promise<LspHandle | null> => {
  const bridge = (window as unknown as { tex64Lsp?: LspBridge }).tex64Lsp;
  if (!bridge) {
