...
export type BlockWysiwygSettingsOps = {
  ...
  setMathWysiwygAutoSuggest: (value: boolean) => void;
  toggleMathWysiwygPack: (packId: string) => void;
};
...
export const createBlockWysiwygSettingsOps = (runtime: BlockInputRuntime): BlockWysiwygSettingsOps => {
  const wysiwygAutoOptions = Array.from(document.querySelectorAll<HTMLButtonElement>("[data-wysiwyg-auto]"));
  const wysiwygPackOptions = Array.from(document.querySelectorAll<HTMLButtonElement>("[data-wysiwyg-pack]"));
