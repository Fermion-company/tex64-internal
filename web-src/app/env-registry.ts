export type LatexEnvKind = "math" | "table";

export type LatexEnvRegistryEntry = {
  name: string;
  kind: LatexEnvKind;
  package: string;
  discouraged?: boolean;
...
export const normalizeEnvName = (name: string) => name.trim();

export const getEnvBaseName = (name: string) =>
  name.endsWith("*") ? name.slice(0, -1) : name;

export const DEFAULT_ENV_REGISTRY: LatexEnvRegistryEntry[] = [
